...
- [x] Mention two parts of Express that you learned about this week.
      Request and Response and routing

- [x] Describe Middleware?
      An array of functions that get executed in order that they are introduced on the server.

- [x] Describe a Resource?
      A library or database that the server references and provides the client.

- [x] What can the API return to help clients know if a request was successful?
      Responses and HTTP codes

- [x] How can we partition our application into sub-applications?
      By splitting the code into modules with their own functions, and local middleware functions. Then allowing the server to call them as needed, via a router.
...
- [x] Perform CRUD operations on _projects_ and _actions_. When adding an action, make sure the `project_id` provided belongs to an existing `project`. If you try to add an action with an `id` of 3 and there is no project with that `id` the database will return an error. -[x] gets all -[x] gets by id
